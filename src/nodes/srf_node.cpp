...
CLaserOdometry2D::CLaserOdometry2D() : Node("SRF_laser_odom")
{
    ...
    this->declare_parameter<double>("increment_covariance_treshold", std::numeric_limits<double>::max());
    ...
    increment_covariance_treshold_ = this->get_parameter("increment_covariance_treshold").get_value<double>();
...
void CLaserOdometry2D::publish_pose_from_SRF()
{
    ...
    else
    {
        ...
        if (cov_odo_2d_diag > increment_covariance_treshold_ || cov_odo_2d_diag != cov_odo_2d_diag)
        {
