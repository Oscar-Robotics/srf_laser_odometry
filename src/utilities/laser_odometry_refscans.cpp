...
void SRF_RefS::initialize(unsigned int size, float FOV_rad, unsigned int odo_method)
{
    ...
    const unsigned int pyr_levels = std::round(std::log2(std::round(float(width) / float(cols)))) + ctf_levels;
...
void SRF_RefS::create_scan_pyramid()
{
...
void SRF_RefS::calculate_range_derivatives()
{
...
void SRF_RefS::solve_system_quad_residuals_3_scans()
{
    ...
    for (unsigned int u = 1; u < cols_i - 1; u++)
    {
        if (null_12(u) == false)
        {
            ...
            // Fill the matrix A
            A(cont, 0) = tw * (std::cos(tita) + dtita_12(u) * kdtita * std::sin(tita) / range_12[image_level](u));
            A(cont, 1) = tw * (std::sin(tita) - dtita_12(u) * kdtita * std::cos(tita) / range_12[image_level](u));
            A(cont, 2) = tw * (-yy_12[image_level](u) * std::cos(tita) + xx_12[image_level](u) * std::sin(tita) -
                               dtita_12(u) * kdtita); //?????
            B(cont) = tw * (-dt_12(u));
        ...
        if (null_13(u) == false)
        {
            ...
            // Fill the matrix A
            A(cont, 0) = tw * (std::cos(tita) + dtita_13(u) * kdtita * std::sin(tita) / range_13[image_level](u));
            A(cont, 1) = tw * (std::sin(tita) - dtita_13(u) * kdtita * std::cos(tita) / range_13[image_level](u));
            A(cont, 2) = tw * (-yy_13[image_level](u) * std::cos(tita) + xx_13[image_level](u) * std::sin(tita) -
                               dtita_13(u) * kdtita);
            B(cont) = tw * (-dt_13(u));
...
void SRF_RefS::solve_system_smooth_trunc_quad_3_scans()
{
    A.resize(num_valid_range, 3);
    ...
    for (unsigned int u = 1; u < cols_i - 1; u++)
    {
        ...
        if (null_12(u) == false)
        {
            ...
            A(cont, 2) =
                tw * (-yy_12[image_level](u) * cos_tita + xx_12[image_level](u) * sin_tita - dtita_12(u) * kdtita);
            B(cont) = tw * (-dt_12(u));
        ...
        if (null_13(u) == false)
        {
            ...
            A(cont, 2) =
                tw * (-yy_13[image_level](u) * cos_tita + xx_13[image_level](u) * sin_tita - dtita_13(u) * kdtita);
            B(cont) = tw * (-dt_13(u));
...
void SRF_RefS::solve_system_smooth_trunc_quad_only_1_3()
{
    ...
    aux_vector.clear();
    for (unsigned int k = 0; k < res.rows(); k++)
        aux_vector.push_back(std::abs(res(k) - res_median));
    std::sort(aux_vector.begin(), aux_vector.end());
...
void SRF_RefS::solve_system_smooth_trunc_quad_only_1_2()
{
    unsigned int valid_here = 0;
    for (unsigned int u = 1; u < cols_i - 1; u++)
    {
...
void SRF_RefS::perform_warping()
{
    ...
    for (unsigned int j = 0; j < cols_i; j++)
    {
        if (range_1[image_level](j) > 0.f)
        {
            ...
            const float x_w =
                acu_trans(0, 0) * xx_1[image_level](j) + acu_trans(0, 1) * yy_1[image_level](j) + acu_trans(0, 2);
            const float y_w =
                acu_trans(1, 0) * xx_1[image_level](j) + acu_trans(1, 1) * yy_1[image_level](j) + acu_trans(1, 2);
            ...
            if ((uwarp >= 0.f) && (uwarp < cols_lim))
            {
                ...
                {
                    int index = static_cast<int>(std::round(uwarp));
...
void SRF_RefS::perform_best_warping()
{
    Eigen::Matrix3f acu_trans;
    acu_trans.setIdentity();
    for (unsigned int i = 0; i <= level; i++)
    ...
    for (unsigned int u = 0; u < cols_i; u++)
    {
        if (range_1[image_level](u) != 0.f)
        {
            // Transform point to the warped reference frame
            x_trans(u) =
                acu_trans(0, 0) * xx_1[image_level](u) + acu_trans(0, 1) * yy_1[image_level](u) + acu_trans(0, 2);
            y_trans(u) =
                acu_trans(1, 0) * xx_1[image_level](u) + acu_trans(1, 1) * yy_1[image_level](u) + acu_trans(1, 2);
    ...
    for (unsigned int u = 0; u < cols_i - 1; u++)
    {
        ...
        else if (floorf(u_trans(u)) != floorf(u_trans(u + 1)))
        {
            ...
            for (unsigned int u_segment = u_l + 1; (u_segment <= u_r) && (u_segment < cols_i) && (u_segment >= 0);
                 u_segment++)
            {
                const float range_interp =
                    ((u_segment - u_trans_l) * range_r + (u_trans_r - u_segment) * range_l) / (u_trans_r - u_trans_l);
...
void SRF_RefS::warp_scan_3_to_2()
{
    ...
    for (unsigned int i = 0; i < ctf_levels; i++)
    {
        ...
        for (unsigned int u = 0; u < cols_i; u++)
        {
            ...
            {
                // Transform point to the warped reference frame
                x_trans(u) = acu_trans_inv(0, 0) * xx_3[image_level](u) + acu_trans_inv(0, 1) * yy_3[image_level](u) +
                             acu_trans_inv(0, 2);
                y_trans(u) = acu_trans_inv(1, 0) * xx_3[image_level](u) + acu_trans_inv(1, 1) * yy_3[image_level](u) +
                             acu_trans_inv(1, 2);
                range_trans(u) = sqrtf(square(x_trans(u)) + square(y_trans(u)));
                const float tita_trans = std::atan2(y_trans(u), x_trans(u));
                u_trans(u) = kdtita * (tita_trans + 0.5f * fovh) - 0.5f;
        ...
        for (unsigned int u = 0; u < cols_i - 1; u++)
        {
            ...
            else if (floorf(u_trans(u)) != floorf(u_trans(u + 1)))
            {
                ...
                for (unsigned int u_segment = u_l + 1; (u_segment <= u_r) && (u_segment < cols_i) && (u_segment >= 0);
                     u_segment++)
                {
                    const float range_interp = ((u_segment - u_trans_l) * range_r + (u_trans_r - u_segment) * range_l) /
                                               (u_trans_r - u_trans_l);
...
void SRF_RefS::odometry_calculation()
{
    //==================================================================================
    ...
    for (unsigned int i = 0; i < ctf_levels; i++)
    {
        ...
        for (unsigned int k = 0; k < 3; k++)
        {
            ...
            if ((i == 0) && (k == 0))
            {
                ...
                perform_best_warping();

            // 2. Calculate inter coords
            calculate_coord();

            // 3. Compute derivatives
            calculate_range_derivatives();
            ...
            {
                if (method == 0)
                    solve_system_smooth_trunc_quad_only_1_2();
                else if (method == 1)
                    solve_system_smooth_trunc_quad_only_1_3();
                else
                {
                    if (new_ref_scan == true)
                        ...
                        solve_system_smooth_trunc_quad_3_scans();
                }
            }

            // 6. Filter solution
            filter_level_solution();
    ...
    update_reference_scan();
}

void SRF_RefS::filter_level_solution()
{
    ...
    {
        kai_b_fil(i, 0) = (kai_b(i, 0) + (cf * eigensolver.eigenvalues()(i, 0) + df) * kai_b_old(i, 0)) /
...
void SRF_RefS::update_reference_scan()
{
    ...
    if (cols < 500)
        keyscan_out_region =
            -14.92 * square(trans_2) - 7.617 * trans_2 * trans + 2.307 * trans_2 - 0.3149 * trans + 0.1054 - rot;
    else
        keyscan_out_region =
            -10.66 * square(trans_2) + 11.81 * trans_2 * trans - 4.371 * trans_2 + 0.5319 * trans + 0.2042 - rot;
